...
def load_config(): # 建议把这个函数也搬到 main.py 或建立共用工具类
    ...
    try:
        while True:
            ...
            if cfg:
                ...
                # 同步 Brain 参数
                brain.Kp = cfg.get("Kp", 0.4)
                brain.Ki = cfg.get("Ki", 0.01)
                brain.Kd = cfg.get("Kd", 0.1)
                brain.voltage_threshold = cfg.get("Critical_V", 1.2)
                pass
