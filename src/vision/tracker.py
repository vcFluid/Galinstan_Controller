...
class GalinstanTracker: # 定义一个名为 GalinstanTracker 的类，[用于负责跟踪液态金属球的**位置**和**运动状态**]，其包含了一个初始化方法 __init__ 和一个处理图像帧的方法 process_frame，以及一些**私有方法**用于图像预处理、分割和提取质心位置。
    ...
    def process_frame(self, frame, debug = False): 
        ...
        # 3. 提取 (几何中心/物理过滤)
        pos = self._find_blob(mask, debug=debug) # pos 是通过调用私有方法 _find_blob（定义在下方） 对分割后的图像 mask 进行提取处理后的结果。这个提取步骤通常包括计算液态金属球的几何中心（质心）并进行物理过滤，以确保提取到的位置是合理的，并且符合预期的物理特性。
...
# 1. 寻找轮廓
        ...
        # 准备调试画布（如果开启调试）
        debug_canvas = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR) if debug else None
        ...
        for cnt in contours:
            ...
            # --- 物理约束过滤门槛 ---
            # 1. 面积门槛：排除过小杂质或过大阴影
            # 2. 圆度门槛：排除长条状干扰（如划痕、长影）
            if 100 < area < 10000 and circularity > 0.6:
                ...
                # 3. 运动连续性约束：距离上一帧位置不能产生“瞬间漂移”
                if self.last_pos is not None:
                    dist = ((cX - self.last_pos[0])**2 + (cY - self.last_pos[1])**2)**0.5
                    # 假设物理最大位移为 150 像素（根据 FPS 和液滴最大预期速度设定）
                    if dist > 150: 
                        if debug: cv2.drawContours(debug_canvas, [cnt], -1, (0, 255, 255), 1) # 黄色表示位移超限
                        continue
                else:
                    dist = 0

                # 在候选者中选择最接近上一帧位置的（即“追踪”逻辑）
                if dist < min_dist:
                    min_dist = dist
